...
def plot_trajectories(trajectories: list[np.ndarray], stereotype_poles: np.ndarray):
    ...
    if pole_dim == traj_dim and len(stereotype_poles) > 0:
        ...
        # Ensure it is a numpy array (in case UMAP returns sparse)
        if hasattr(reduced, "toarray"):
            reduced = reduced.toarray() # type: ignore
        ...
        if hasattr(reduced_traj, "toarray"):
            reduced_traj = reduced_traj.toarray() # type: ignore
            
        reduced_traj = np.asarray(reduced_traj)
